...
function startGame(){
  gameStatus = "Start";
  document.querySelector("#status").innerHTML = "Começando o jogo";
}

// modifique o status do jogo se qualquer tecla for pressionada
function changeGameStatud(character){
  if(xNose !== "" && gameConfig.status==="start" && gameStatus==="Start") {
    world_start.play();
...
function jumping(character){
	if((yNose < 200 && character.live) || (touchIsDown&&character.live)){
